...
export default function FranchiseStoresStatusPage() {
  ...
  const loadStoreStatuses = async () => {
    try {
      ...
      const timestamp = new Date().getTime()
      const response = await fetch(`/api/franchise/stores/status?t=${timestamp}`, {
        cache: 'no-store',
        headers: {
          'Cache-Control': 'no-cache',
        },
      ...
      if (response.ok) {
        ...
        data.data?.forEach((store: any) => {
          ...
          console.log('  - has_product_inflow_today:', store.has_product_inflow_today)
          console.log('  - has_storage_photos:', store.has_storage_photos)
          console.log('  - storage_photos:', store.storage_photos, `(length: ${store.storage_photos?.length || 0})`)
        })
  ...
  const handleViewReceivedRequests = async (storeId: string) => {
    ...
    try {
      const timestamp = new Date().getTime()
      const response = await fetch(`/api/franchise/stores/${storeId}/requests?t=${timestamp}`, {
        cache: 'no-store',
        headers: {
          'Cache-Control': 'no-cache',
        },
  ...
  const loadStoreCompletedRequests = async (storeId: string) => {
    try {
      const response = await fetch(`/api/franchise/stores/${storeId}/requests`)
      const data = await response.json()
      if (response.ok) {
        const requestsData = data.data || { received: [], in_progress: [], completed: [], rejected: [] }
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      ...
      for (const store of targets) {
        ...
        const response = await fetch('/api/franchise/requests', {
          method: 'POST',
  ...
  const handleApproveRequest = async (requestId: string) => {
    try {
      const response = await fetch(`/api/franchise/requests/${requestId}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleUpdateRequest = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/franchise/requests/${requestId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category: requestFormData.category,
  ...
  const handleConfirmLostItem = async (lostItemId: string) => {
    ...
    try {
      ...
      console.log('Making API call to:', `/api/franchise/lost-items/${lostItemId}/confirm`)
      
      const response = await fetch(`/api/franchise/lost-items/${lostItemId}/confirm`, {
        method: 'PATCH',
        headers: {
  ...
  const handleCompleteStoreProblem = async (problemId: string) => {
    try {
      ...
      const response = await fetch(`/api/franchise/problem-reports/${problemId}/complete`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const refreshStore = async (storeId: string, e?: React.MouseEvent) => {
    ...
    try {
      const response = await fetch(`/api/franchise/stores/${storeId}/status`)
      const data = await response.json()
  ...
  const stats = useMemo(() => {
    ...
    const productInflowCount = storeStatuses.filter((s) => s.has_product_inflow_today).length
    return {
      ...
      productInflow: productInflowCount,
    }
  }, [storeStatuses])
  ...
  useEffect(() => {
    if (showAllProblemsModal) {
      const loadData = async () => {
        ...
        // 문제가 있는 매장만 필터링 (has_problem이 true인 모든 매장)
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-purple-500">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 mb-1">제품 입고</div>
            {stats.productInflow > 0 && (
              <button
                onClick={() => {
                  // 제품 입고가 있는 모든 매장의 인벤토리 사진을 보여주는 모달 열기
                  // 전체 매장 중 제품 입고가 있는 매장들을 필터링하여 모달에 표시
                  const storesWithProductInflow = storeStatuses.filter(s => s.has_product_inflow_today)
                  if (storesWithProductInflow.length > 0) {
                    // 첫 번째 매장의 인벤토리 모달을 열고, 사용자가 다른 매장도 볼 수 있도록 처리
                    handleOpenInventoryModal(storesWithProductInflow[0])
                  }
                }}
          ...
          <div className="text-2xl font-bold text-gray-900">{stats.productInflow} 곳</div>
        </div>
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-red-500">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <div className="text-sm text-gray-600">문제발생</div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      {status.has_product_inflow_today && (
                        <div 
                          className={`space-y-1 rounded p-2 -mx-2 transition-colors ${
                            isInactive ? 'bg-gray-100 cursor-not-allowed' : 'bg-green-50 cursor-pointer hover:bg-green-100'
                          }`}
                          onClick={(e) => { e.stopPropagation(); if (!isInactive) handleOpenInventoryModal(status) }}
                        >
                          <div className="flex items-center gap-2">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      {isInactive ? (
                        ...
                        <Link
                          href={`/franchise/stores/${status.store_id}/detail`}
                    ...
                    >
                      ...
                      >
                        <div className="flex flex-col gap-2">
                          ...
                          {/* 관리시작 시간 (관리일이고 출근한 경우만, 비활성 제외) */}
                          {!isInactive && status.is_work_day && status.clock_in_time && (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={async () => {
                                try {
                                  const response = await fetch(`/api/franchise/requests/${request.id}`, {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                        ...
                        <>
                          ...
                          <div className="flex gap-2 mt-4">
                            ...
                            <button
                              onClick={async () => {
                                try {
                                  const response = await fetch(`/api/franchise/requests/${request.id}/status`, {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                      status: 'in_progress',
