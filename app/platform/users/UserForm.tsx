...
type PlatformUserFormStore = Pick<Store, 'id' | 'name' | 'company_id'>

interface UserFormProps {
  user: UserWithCompany
  stores: PlatformUserFormStore[]
  companies: Company[]
  initialStoreIds?: string[]
  onSuccess: (user: UserWithCompany) => void
  onCancel: () => void
}

export default function UserForm({ user, stores, companies, initialStoreIds = [], onSuccess, onCancel }: UserFormProps) {
  ...
  const [employmentContractDate, setEmploymentContractDate] = useState(
    user.employment_contract_date ? user.employment_contract_date.split('T')[0] : ''
  )
  const [salaryDate, setSalaryDate] = useState(user.salary_date?.toString() || '')
  const [salaryAmount, setSalaryAmount] = useState(user.salary_amount?.toString() || '')
