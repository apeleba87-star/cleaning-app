...
type PlatformCreateUserFormStore = Pick<Store, 'id' | 'name' | 'company_id'>

interface CreateUserFormProps {
  stores: PlatformCreateUserFormStore[]
  companies: Company[]
  onSuccess: () => void
  onCancel: () => void
}

export default function CreateUserForm({ stores, companies, onSuccess, onCancel }: CreateUserFormProps) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="border border-gray-300 rounded-md p-4 max-h-60 overflow-y-auto">
            {!selectedCompanyId ? (
              <p className="text-gray-500 text-sm">회사를 먼저 선택하세요.</p>
            ) : availableStores.length === 0 ? (
              <p className="text-gray-500 text-sm">선택한 회사에 등록된 매장이 없습니다.</p>
            ) : (
