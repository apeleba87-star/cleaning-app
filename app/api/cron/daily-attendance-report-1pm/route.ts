...
/**
 * 매일 오후 1시에 실행되는 Cron Job
 * 어제 오후 1시 기준으로 모든 매장(일반+야간)의 출근 현황 리포트를 생성합니다.
 * 
 * Vercel Cron Job 설정:
 * - Schedule: "0 13 * * *" (매일 오후 1시 KST)
 * - Path: /api/cron/daily-attendance-report-1pm
 */
export async function GET(request: NextRequest) {
  try {
    ...
    for (const company of companies) {
      try {
        // 모든 매장 조회 (일반+야간, 근무일 정보 및 야간 근무 시간 포함)
        ...
        if (nightStores.length > 0) {
          const nightStoreAttendanceQueries = nightStores.map(async (store) => {
        ...
        // 리포트를 데이터베이스에 저장하거나 알림을 보낼 수 있습니다
        // 여기서는 로그만 출력합니다
    ...
    console.error('Error in GET /api/cron/daily-attendance-report-1pm:', error)
