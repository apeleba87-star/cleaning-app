...
/**
 * 매일 오전 6시에 실행되는 Cron Job
 * 어제 오후 1시 기준으로 일반 매장(야간 매장 제외)의 출근 현황 리포트를 생성합니다.
 * 
 * Vercel Cron Job 설정:
 * - Schedule: "0 6 * * *" (매일 오전 6시 KST)
 * - Path: /api/cron/daily-attendance-report-6am
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error in GET /api/cron/daily-attendance-report-6am:', error)
